...
void FreecellView::mousePressEvent(QMouseEvent *e)
{
  ...
  // column ?
	...
	if(x==selected_card.x && y==selected_card.y && card_selected && x>=0 && game_active)
	{
		// remove selection of this card
		card_selected = 0;
		if(selected_card.where==FIELD)
			p.drawPixmap(20+x*90, 160+y*25, cardpics[cards.getCard(x, y)],
										0, 0, CARD_WIDTH-1, CARD_HEIGHT);
		else
		if(selected_card.where==BOX)
			p.drawPixmap(20+80*x+(x/4)*70+1, 41, cardpics[cards.getBoxCard(x)]);
	}
	...
	if(game_active)	
  	{
          	...
          	if(y==-1 && x<*(parent_class->opt.num_freecells) && x>=0)
          	{
          		if(!card_selected)
          		{
          			...
          			if(cards.getBoxCard(x)!=NO_CARD) card_selected = true;
          		}
          		else
          		{
          			if(selected_card.x==x&&selected_card.where==BOX) card_selected = false;
          			else
          				if(selected_card.where==FIELD)
          				{
		...
		if(rest>0&&game_active&&!card_selected) checkAutoMoves(&p);
...
void FreecellView::paintEvent(QPaintEvent *event)
{
	...
	if(! *(parent_class->opt.background_enabled) )
		p.fillRect(event->rect().x(), event->rect().y(),
								event->rect().width(), event->rect().height(), QBrush(QColor(0,128,0)));
	else
			...
			for(j=0;j<15;j++)
				if(cards.getCard(i,j)!=NO_CARD)
					if(20+i*90+CARD_WIDTH>(event->rect()).left())
						p.drawPixmap(20+i*90, 160+j*25, cardpics[cards.getCard(i,j)],
													0, 0, CARD_WIDTH-1, CARD_HEIGHT);
	...
	for(i=0;i<8;i++)
		if(i<*(parent_class->opt.num_freecells) || i>3)
			if(cards.getBoxCard(i)!=NO_CARD)
				if(20+80*i+(i/4)*70+1+CARD_WIDTH > (event->rect()).left())
					p.drawPixmap(20+80*i+(i/4)*70+1, 41, cardpics[cards.getBoxCard(i)],
												0, 0, CARD_WIDTH-1, CARD_HEIGHT);
...
void FreecellView::getCardPosition(int mx, int my, int *x, int *y)
{
	*x = *y = -1;
	...
	if( ( (mx>20 && mx<90) || (mx>100 && mx<170) || (mx>180 && mx<250) ||
				(mx>260 && mx<330) ) && (my>40 && my<140) )
	{
		*x = (mx-20)/80;
		*y = -1;
	}
	...
	if( ( (mx>430 && mx<500) || (mx>510 && mx<580) || (mx>590 && mx<660) ||
				(mx>670 && mx<740) ) && (my>40 && my<140) )
	{
		*x = (mx-410)/80+4;
		*y = -1;
	}	
	...
	for(i=0;i<*(parent_class->opt.num_freecells);i++)
		if(cards.getBoxCard(i)!=NO_CARD)
			for(j=0;j<8;j++)
			{
				...
				if(cards.checkMoveFromBox(i, j, n)) turns++;
			}
	...
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
			if(j!=i)
			{
				...
				if(cards_i>0)
					if(cards_j==0
							&& !cards.checkMove(i, cards_i-1, i, cards_i-1) ) turns++;
					else
						if(cards.checkMove(i, cards_i-1, j, cards_j)
		...
		if(cards.getNumCardsAtCol(i)>0)
			for(j=0;j<8;j++)
				if(j<*(parent_class->opt.num_freecells) || j>3)
					if(cards.checkMoveToBox(i, cards.getNumCardsAtCol(i)-1, j))
						turns++;
...
void FreecellView::drawMovingCard(int sx, int sy, int dx, int dy, QPainter *p){
	...
	if(parent_class->protocol_fd!=NULL)
	{
		...
		if(sy==-1) d[0] = l[sx]; else d[0] = l[sx+8];
		if(dy==-1) d[1] = l[dx]; else d[1] = l[dx+8];
		fprintf(parent_class->protocol_fd, "%s ", d);
		if(moves%10==0) fprintf(parent_class->protocol_fd, "\n");
	}
	...
	switch(sy)
	{
		...
		default:  // move card from column...
				...
				if(! *(parent_class->opt.background_enabled))
					p->fillRect(20+sx*90, 160+sy*25, CARD_WIDTH-1, CARD_HEIGHT, QBrush(QColor(0,128,0)));
				else
					p->drawPixmap(20+sx*90, 160+sy*25, background_picture,
													20+sx*90, 160+sy*25, CARD_WIDTH-1, CARD_HEIGHT);
				if(sy-1>=0)
			  	if(cards.getCard(sx, sy-1)!=NO_CARD)
							p->drawPixmap(20+sx*90, 160+(sy-1)*25,	cardpics[cards.getCard(sx, sy-1)],
														0, 0, CARD_WIDTH-1, CARD_HEIGHT);
  		...
  		if(cards.getNumCardsAtCol(i)>0)
  			for(j=4;j<8;j++)
  				if(cards.checkMoveToBox(i, cards.getNumCardsAtCol(i)-1, j))
  				{
  					...
  					for(k=4;k<8;k++)
  						if(cards.getBoxCard(k)==NO_CARD)	o[k-4]=-1;
  							else o[k-4] = cards.getBoxCard(k)/4;
						
  					if( (o[0]==NO_CARD||o[1]==NO_CARD||o[2]==NO_CARD||o[3]==NO_CARD||
						...
						{
							for(k=0;k<4;k++)
								if(k!=j) if(o[k]>c+1) move=false;
						}
  						  						  					
  					if(  move  )  // 0 = "A", 12 = "2"
  					{
	  					cards.moveCard(i, cards.getNumCardsAtCol(i)-1, j);
  						drawMovingCard(i, cards.getNumCardsAtCol(i), j, -1, p);
  						rest--;
  						moves++;
  						break;
  					}
